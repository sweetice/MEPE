...
class Agent():
    def __init__(self, args, env):
        ...
        self.popro_coef = args.popro_coef
        self.algo = args.algo
        print("Algorithm name: ", self.algo)
    ...
    def learn(self, mem):
        ...
        mask = self.online_net.dropout(self.online_net.ones)
        log_ps, _, _ = self.online_net(states, log=True, representation=True, mask=mask)
        _, z_anch = self.online_net(aug_states_1, log=True, mask=mask)
        _, z_target = self.momentum_net(aug_states_2, log=True, mask=mask)
        ...
        with torch.no_grad():
            ...
            if self.algo == "mepe":
                pns, _, _ = self.target_net(next_states, representation=True ,mask=mask)
            else:
                pns, _, _ = self.target_net(next_states, representation=True)  # Probabilities p(s_t+n, ·; θtarget)
            pns_a = pns[range(
